# Documentação de Arquitetura 

## Estrutura de Diretórios do Projeto

```
cdi-ipca-analysis/
├── main.py                      # Ponto de entrada da aplicação
├── README.md                    # Documentação do projeto
├── LICENSE                      # Licença do projeto
├── ARCHITECTURE.md              # Documentação de arquitetura
├── CONTRIBUTING.md              # Guia de contribuição
├── REQUIREMENTS.txt             # Dependências do projeto
│
├── data/                        # Diretório de dados
│   ├── raw/                     # Dados brutos da API
│   │   ├── monthly_cdi/        # CDI mensal
│   │   ├── yearly_cdi/         # CDI anual
│   │   └── ipca/               # IPCA mensal
│   └── processed/              # Dados processados
│       ├── cdi_data.xlsx       # Base CDI
│       └── ipca_data.xlsx      # Base IPCA
│
├── src/                        # Código fonte
│   ├── config.py              # Configurações da API
│   ├── pipeline.py            # Orquestrador da pipeline
│   │
│   ├── fetch/                 # Módulos de coleta
│   │   ├── fetch_cdi.py      # Busca CDI
│   │   └── fetch_ipca.py     # Busca IPCA
│   │
│   ├── transform/             # Módulos de transformação
│   │   ├── cdi_transform.py  # Cálculos CDI
│   │   ├── ipca_transform.py # Cálculos IPCA
│   │   └── tax_rates.py      # Tabela IR (implementação futura)
│   │
│   ├── storage/               # Módulos de persistência
│   │   ├── excel_cdi.py      # Persistência CDI
│   │   ├── excel_ipca.py     # Persistência IPCA
│   │   └── schema.py         # Schemas de dados
│   │
│   ├── models/                # Modelos de dados
│   │   └── cdb.py            # Modelo CDB
│   │
│   └── utils/                 # Utilitários
│       ├── date_utils.py     # Utilidades de data
│       └── directory_utils.py # Utilidades de diretório
│
├── scripts/                   # Scripts auxiliares
│   ├── run_pipeline.bat      # Script Windows
│   └── run_pipeline.sh       # Script Unix/Linux
│
├── notebooks/                 # Jupyter notebooks (implementação futura)
└── tests/                     # Testes unitários (implementação futura)
```


## Diagrama de Arquitetura

```
┌─────────────────────────────────────────────────────────────────┐
│                     USUÁRIO / SCHEDULER                         │
│               (Execução Manual ou Automática)                   │
└─────────────────────────────┬───────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│                          MAIN.PY                                │
│                    ┌──────────────────┐                         │
│                    │  Argumentos CLI  │                         │
│                    │  ─ mode          │                         │
│                    │  ─ persistence   │                         │
│                    │  ─ year          │                         │
│                    │  ─ clear-data    │                         │
│                    └──────────────────┘                         │
└────────────────────────────┬────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────┐
│                         PIPELINE.PY                             │
│                    ┌──────────────────┐                         │
│                    │  Orquestrador    │                         │
│                    │  ─ _run_month    │                         │
│                    │  ─ _run_year     │                         │
│                    │  ─ _run_backfill │                         │
│                    └──────────────────┘                         │
└─┬─────────────────┬───────────────┬───────────────┬─────────────┘
  │                 │               │               │
  │ FETCH           │ TRANSFORM     │ LOAD          │ UTILS
  │                 │               │               │
  ▼                 ▼               ▼               ▼
┌────────┐    ┌──────────┐    ┌──────────┐    ┌──────────┐
│ API    │    │ Cálculos │    │  Excel   │    │  Date    │
│ BCB    │    │ CDI/IPCA │    │  SQLite  │    │ Business │
│        │    │ Impostos │    │  (futuro)│    │   Days   │
│ fetch_ │    │ cdi_     │    │ excel_   │    │ date_    │
│ cdi.py │    │ transform│    │ cdi.py   │    │ utils.py │
│ fetch_ │    │ ipca_    │    │ excel_   │    │ directory│
│ ipca.py│    │ transform│    │ ipca.py  │    │ _utils.py│
│        │    │ tax_     │    │          │    │          │
│        │    │ rates.py │    │          │    │          │
└───┬────┘    └────┬─────┘    └────┬─────┘    └──────────┘
    │              │               │
    ▼              ▼               ▼
┌─────────────────────────────────────────────────────────────────┐
│                      ARMAZENAMENTO                              │
│  ┌───────────────────┐          ┌───────────────────┐           │
│  │   data/raw/       │          │  data/processed/  │           │
│  │  ─ monthly_cdi/   │          │  ─ cdi_data.xlsx  │           │
│  │  ─ yearly_cdi/    │          │  ─ ipca_data.xlsx │           │
│  │  ─ ipca/          │          │                   │           │
│  │  (JSON backups)   │          │  (Dados limpos)   │           │
│  └───────────────────┘          └───────────────────┘           │
└─────────────────────────────────────────────────────────────────┘
```

## Fluxo de Dados (ETL)

```
┌──────────┐
│ Extract  │  API do Banco Central do Brasil
└────┬─────┘  ↓
     │        • Série 12: CDI Taxa de juros Diário (não usada)
     │        • Série 4391: CDI Mensal
     │        • Série 4392: CDI Anual
     │        • Série 433: IPCA Mensal
     ▼
┌──────────────────────────────────────────┐
│          JSON Response                   │
│  {                                       │
│    "data": "01/01/2025",                 │
│    "valor": "0.87"                       │
│  }                                       │ 
└────┬─────────────────────────────────────┘
     │
     ▼
┌──────────┐
│Transform │  Processamento
└────┬─────┘  ↓
     │        • Conversão de tipos
     │        • Cálculo de fatores diários
     │        • Taxas acumuladas
     │        • Validações
     ▼
┌──────────────────────────────────────────┐
│       Dados Estruturados                 │
│  {                                       │
│    "year": 2025,                         │
│    "month": 1,                           │
│    "cdi_monthly_rate": 0.87,             │
│    "cdi_annual_rate": 10.65              │
│  }                                       │
└────┬─────────────────────────────────────┘
     │
     ▼
┌──────────┐
│   Load   │  Persistência
└────┬─────┘  ↓
     │        • Excel (atual)
     │        • SQLite (futuro)
     │        • PostgreSQL (futuro)
     │        • Backup JSON
     ▼
┌───────────────────────────────────────────┐
│        Armazenamento Final                │
│                                           │
│  📊 cdi_data.xlsx                         │
│  📊 ipca_data.xlsx                        │
│  📄 monthly_cdi_2025-01.json              │
│  📄 yearly_cdi_2025-01.json               │
│  📄 ipca_2025-01.json                     │
└───────────────────────────────────────────┘
```

## Modos de Execução

### 1️⃣ Month Mode (Modo Mensal)

```
┌─────────────────────────────────────────┐
│  python main.py --mode month            │
└──────────────┬──────────────────────────┘
               │
               ▼
        ┌──────────────┐
        │ Data Atual?  │
        └──────┬───────┘
               │
         ┌─────▼─────┐
         │ Dia Útil? │
         └─────┬─────┘
          Não  │  Sim
         ┌─────▼────────┐
         │Já Processado?│
         └─────┬────────┘
          Não  │  Sim
         ┌─────▼──────┐
         │ Fetch CDI  │
         │ Fetch IPCA │
         └─────┬──────┘
               │
         ┌─────▼─────┐
         │ Transform │
         └─────┬─────┘
               │
         ┌─────▼─────┐
         │   Load    │
         └─────┬─────┘
               │
               ▼
           Sucesso!
```

### 2️⃣ Yearly Mode (Modo Anual)

```
┌─────────────────────────────────────────────┐
│  python main.py --mode yearly --year 2025   │
└──────────────┬──────────────────────────────┘
               │
               ▼
      ┌────────────────┐
      │ Range: 01/01   │
      │   até: 31/12   │
      └────────┬───────┘
               │
               ▼
      ┌────────────────┐
      │ Fetch ALL Data │
      │  CDI + IPCA    │
      └────────┬───────┘
               │
               ▼
      ┌────────────────────┐
      │ Para cada entrada: │
      │ ├─ Já processado?  │
      │ │   └─ Sim: Pula   │
      │ │   └─ Não: ↓      │
      │ ├─ Transform       │
      │ └─ Load            │
      └────────┬───────────┘
               │
               ▼
          Ano completo!
```

### 3️⃣ Backfill Mode (Preenchimento de lacunas)

```
┌─────────────────────────────────────────┐
│  python main.py --mode backfill         │
└──────────────┬──────────────────────────┘
               │
               ▼
      ┌────────────────┐
      │ Scan data/raw/ │
      │ Encontra datas │
      │ já processadas │
      └────────┬───────┘
               │
               ▼
      ┌────────────────┐
      │ Identifica     │
      │ Min/Max Date   │
      └────────┬───────┘
               │
               ▼
      ┌────────────────┐
      │ Fetch Range    │
      │ Completo       │
      └────────┬───────┘
               │
               ▼
      ┌────────────────────┐
      │ Para cada data:    │
      │ ├─ Existe?         │
      │ │   └─ Sim: Pula   │
      │ │   └─ Não: ↓      │
      │ ├─ Transform       │
      │ └─ Load            │
      └────────┬───────────┘
               │
               ▼
        Lacunas preenchidas!
```

## Modelo de Dados

### CDI (Excel)

```
┌──────┬───────┬──────────────────┬──────────────────┐
│ year │ month │ cdi_monthly_rate │ cdi_annual_rate  │
├──────┼───────┼──────────────────┼──────────────────┤
│ 2025 │   1   │      0.87        │      10.65       │
│ 2025 │   2   │      0.91        │      11.20       │
│ 2025 │   3   │      0.85        │      10.40       │
└──────┴───────┴──────────────────┴──────────────────┘
```

### IPCA (Excel)

```
┌──────┬───────┬──────────────────┐
│ year │ month │ipca_monthly_rate │
├──────┼───────┼──────────────────┤
│ 2025 │   1   │      0.0042      │   
│ 2025 │   2   │      0.0038      │  
│ 2025 │   3   │      0.0045      │      
└──────┴───────┴──────────────────┘
```

### JSON Backup (Raw Data)

```json
{
  "reference_date": "2025-01",
  "type": "monthly_cdi",
  "value": 0.87
}
```

## Casos de Uso

### 1. Investidor Individual
```
Objetivo: Acompanhar rendimento de CDB
├─ Executar: python main.py (diário)
├─ Analisar: Excel com histórico completo
└─ Calcular: IR, rendimento líquido
```

### 2. Analista Financeiro
```
Objetivo: Análise histórica e comparações
├─ Executar: python main.py --mode yearly --year 2024
├─ Exportar: Dados para ferramentas de análise
└─ Visualizar: Gráficos CDI vs IPCA
```

### 3. Pesquisador
```
Objetivo: Estudos de correlação econômica
├─ Executar: Backfill para dados históricos
├─ Integrar: Pandas/NumPy para análises
└─ Publicar: Resultados de pesquisa
```

### 4. Desenvolvedor
```
Objetivo: Integrar com outras aplicações
├─ Usar: Como módulo Python
├─ Acessar: Dados via Excel ou API
└─ Estender: Novos indicadores
```

## Stack Tecnológico

```
┌─────────────────────────────────────────┐
│           Frontend (Futuro)             │
│   Dashboard Web - Streamlit/Dash        │
└──────────────┬──────────────────────────┘
               │
┌──────────────▼──────────────────────────┐
│         Camada de Aplicação             │
│                                         │
│  Python 3.8+                            │
│  ├─ CLI (argparse)                      │
│  ├─ Pipeline (ETL)                      │
│  └─ Models (CDB)                        │
└──────────────┬──────────────────────────┘
               │
┌──────────────▼──────────────────────────┐
│        Bibliotecas Core                 │
│                                         │
│  ├─ Pandas      (Data manipulation)     │
│  ├─ Requests    (HTTP client)           │
│  ├─ OpenPyXL    (Excel I/O)             │
│  ├─ NumPy       (Numerical computing)   │
│  └─ dateutil    (Date utilities)        │
└──────────────┬──────────────────────────┘
               │
┌──────────────▼──────────────────────────┐
│          Data Sources                   │
│                                         │
│  API BCB (https://api.bcb.gov.br)       │
│  ├─ Série 12: CDI Diário                │
│  ├─ Série 4391: CDI Mensal              │
│  └─ Série 433: IPCA                     │
└──────────────┬──────────────────────────┘
               │
┌──────────────▼──────────────────────────┐
│          Persistência                   │
│                                         │
│  ├─ Excel (.xlsx)            [Atual]    │
│  ├─ JSON (backups brutos)    [Atual]    │
│  └─ SQLite (.db)             [Futuro]   │
└─────────────────────────────────────────┘
```

## Roadmap

### Versão 0.1.0 (Atual)
- [x] Pipeline ETL básica
- [x] CDI e IPCA implementados
- [x] Persistência Excel
- [x] Criação dos métodos de execução principais
- [x] Classe CDB
- [x] Documentação básica

### Versão 0.2.0 
- [ ] Revisão dos métodos de execução
- [ ] Estrutura de fetchs escalável e modular
- [ ] Estrutura de persistência modular e escalável
- [ ] Estrutura da pipeline escalável
- [ ] Cálculo de todas as mensurações de CDI e IPCA
- [ ] Testes automatizados (pytest)
- [ ] Notebooks de demonstração e análises básicos

### Versão 0.3.0
- [ ] Metadados na persistência
- [ ] Mais opções de persistência de dados brutos e processados (SQLite, PostgreSQL)
- [ ] Scripts de automação de execução 
- [ ] Mais indicadores
- [ ] Dashboards
- [ ] Logs de execução

### Versões 0.4.0 < 
- [ ] Todos os indicadores principais implementados com todas as mensurações
- [ ] Machine Learning (previsões)
- [ ] Classes de outros títulos
- [ ] Simuladores de investimento
- [ ] Dashboard web interativo completo
- [ ] Documentação completa
- [ ] Notebooks educacionais e tutoriais
- [ ] Logo e identidade visual
- [ ] Possível API REST para acesso aos dados processados
- [ ] Possível GUI


---

**Documentação visual mantida por:** rafaGarciaT
**Última atualização:** Fevereiro 2026

